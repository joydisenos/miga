...
;(function ($, window, document, undefined) {
    ...
    var defaults = {
        ...
        lang: { // Language variables
            cartTitle: "Su carrito de compras",
            checkout: '<i class="fa fa-paper-plane" aria-hidden="true"></i> Finalizar Pedido',
            clear: 'Vaciar carrito',
            subtotal: 'Subtotal:',
            cartRemove: '<i class="fas fa-trash" aria-hidden="true"></i>',
            cartEmpty: 'Tus productos seran cargados<br />aqui'
        },
        submitSettings: {
    ...
    $.extend(SmartCart.prototype, {
            ...
            if (this.options.toolbarSettings.showCheckoutButton) {
                ...
                switch (this.options.toolbarSettings.checkoutButtonStyle) {
                    ...
                    case 'image':
                        ...
                        break;
                    default:
                        btnCheckout = '<button class="btn btn-success btn-lg sc-cart-checkout" type="button" style="width:100%">' + this.options.lang.checkout + '</button> ';
                        break;
                }
        ...
        _setEvents: function () {
            ...
            $(this.options.addCartSelector).on("click", function (e) {
                ...
                //$(this).parents(mi.options.productContainerSelector).attr('data-product-unique-key', p.unique_key);
                var cantidad_productos_carrito = mi.cart.length;
                $('#sc_product_item_' + p.product_id).addClass('sc-added-item').attr('data-product-unique-key', p.unique_key);
                $('#cantidad_productos_carrito').text(cantidad_productos_carrito);
        ...
        _addUpdateCartItem: function (p) {
            ...
            if (elmMain && elmMain.length > 0) {
                ...
                var itemSummary = '<div class="sc-cart-item-summary">Unidades <span class="sc-cart-item-price hidden">' + this._getMoneyFormatted(p[this.options.paramSettings.productPrice]) + '</span>';
                itemSummary += ' <input type="number"  disabled="disabled" min="0" max="1000" class="sc-cart-item-qty " value="' + this._getValueOrEmpty(p[this.options.paramSettings.productQuantity]) + '"/>';
				
                itemSummary += '  <span class="sc-cart-item-amount"><b>' + this._getMoneyFormatted(productAmount) + '</b></span></div>';
